...
function publication_head($query) {
    ...
    if (count($display_tags)) {
        echo '<h5><i class="fa fa-tags fa-lg"></i> ';
    }
...
function must_login($query = null, $msg = 'to view the full article and videos on this page.') {
	...
	echo sprintf('<br><div class="alert alert-warning" role="alert"><p class="lead"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> You must <a href="/wordpress/wp-login.php?redirect_to=%s">login</a> %s</p></div>', get_permalink(), $msg);
}
...
function videoSitemap($dat) {
    ...
    fclose($fileHandle);
    return '<p class="lead">Video sitemap written to <a href="/wordpress/video-sitemap.xml">video-sitemap.xml</a></p>';
}
...
function download_brightcove() {
    ...
    $msg .= '<p class="lead">DB downloaded to <a href="/wordpress/BC_DB.json">BC_DB.json</a></p>';
